...
function registerIpc() {
  ...
  ipcMain.handle('diag:export', async (_e, rendererSnapshot) => {
    ...
    const info = {
      app: 'AnniePlayerPlusProVersion beta0.0.3',
  ...
  // 流媒体登录
  ipcMain.handle('stream:loginStatus', (_e, params) => streaming.loginStatus(params));
  ipcMain.handle('stream:neteaseQrCreate', () => streaming.qrCreateNetease());
  ipcMain.handle('stream:neteaseQrCheck', (_e, params) => streaming.qrCheckNetease(params));
  ipcMain.handle('stream:qqSetCookie', (_e, params) => streaming.setQQCookie(params));
  ipcMain.handle('stream:logout', (_e, params) => streaming.logout(params));
