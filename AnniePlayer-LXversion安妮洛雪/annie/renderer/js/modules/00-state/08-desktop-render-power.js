...
function applyRendererPowerMode() {
  ...
  var deep = isDeepBackgroundMode();
  // buffer 尺寸跟随舞台容器（canvas CSS 为容器 100%），避免侧栏/电平栏占位时被非等比拉伸
  var vpEl = document.getElementById('canvas-container');
  var width = deep ? 4 : (vpEl && vpEl.clientWidth ? vpEl.clientWidth : Math.max(1, innerWidth));
  var height = deep ? 4 : (vpEl && vpEl.clientHeight ? vpEl.clientHeight : Math.max(1, innerHeight));
