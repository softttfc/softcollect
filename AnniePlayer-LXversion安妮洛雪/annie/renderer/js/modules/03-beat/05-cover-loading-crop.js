...
function loadCoverFromUrl(directUrl, opts) {
  ...
  img.onerror = function () {
    // 首次加载失败后，移除 crossOrigin 重试（兼容无 CORS 头的 CDN）
    var img2 = new Image(); img2.decoding = 'async';
    img2.onload = function () {
      if (!coverApplyStillCurrent(opts)) return;
      var size = coverTextureSizeForResolution(fx.coverResolution);
      var cv = document.createElement('canvas'); cv.width = cv.height = size;
      var cx = cv.getContext('2d');
      var iw = img2.naturalWidth, ih = img2.naturalHeight, s = Math.min(iw, ih);
      cx.drawImage(img2, (iw - s) / 2, (ih - s) / 2, s, s, 0, 0, size, size);
