...
function mainLoopBackgroundDelayMs() {
  // 主题隐藏（AM/FB2K）：舞台不可见，主循环只需 4fps 心跳维持 prevTime 刷新即可
  if (window.__legacyThemeHidden) return 250;
...
function animate() {
  ...
  // FB2K 主题激活时舞台不可见：跳过重负载渲染仅保心跳（prevTime 刷新，切回无 dt 跳变）
  if (window.__legacyThemeHidden) { prevTime = performance.now(); return; }
  ...
  // 封面粒子组等比适配：舞台过窄时仅整体等比缩小（1:1 原始比例不变，不裁切不变形）。
  // 参考距离取预设基准半径，用户滚轮缩放不受影响；阈值 4.8 = 封面平面边长（PLANE_SIZE）。
  var coverFit = 1;
  if (camera && particles) {
    var fitRefDist = (orbit && orbit.baselineRadius) || camera.position.distanceTo(particles.position);
    var fitVisW = 2 * fitRefDist * Math.tan((camera.fov || 45) * Math.PI / 360) * (camera.aspect || 1);
    coverFit = Math.min(1, (fitVisW * 0.94) / 4.8);
  }
  particles.scale.setScalar(coverFit);
  if (bloomParticles) bloomParticles.scale.setScalar(coverFit);
  if (floatGroup) floatGroup.scale.setScalar(coverFit);
  if (backCoverGroup) backCoverGroup.scale.setScalar(coverFit);
...
function refreshMainRendererViewport(reason) {
  if (typeof camera !== 'undefined' && camera) {
    ...
    var vpEl = document.getElementById('canvas-container');
    var vpW = vpEl && vpEl.clientWidth ? vpEl.clientWidth : Math.max(1, innerWidth);
    var vpH = vpEl && vpEl.clientHeight ? vpEl.clientHeight : Math.max(1, innerHeight);
    camera.aspect = vpW / vpH;
    camera.updateProjectionMatrix();
  }
  if (typeof applyRendererPowerMode === 'function') applyRendererPowerMode();
  if (typeof requestStageLyricCameraSnap === 'function' && (document.fullscreenElement || document.body.classList.contains('desktop-fullscreen'))) {
...
// 舞台容器尺寸变化（窗口缩放、侧栏收放/拖拽、可视化面板开关等）时同步视口，
// 保证渲染 buffer 与容器始终同比例，画面只缩放不变形
