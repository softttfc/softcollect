...
(function () {
  ...
  function applyDeco(deco) {
    ...
    S.rows = rows;
    S.tracks = list;
    // beta0.0.3：路径 → 行号 / 曲目序号双索引，定位与队列查找 O(1)
  ...
  function fb2kQueueIndex() {
    var i = S.trackPathIdx ? S.trackPathIdx.get(state.currentPath) : undefined; // beta0.0.3：O(1)
   ...
   * beta0.0.3：Map O(1) 查找；平滑滚动 + 播放行高亮闪烁 */
