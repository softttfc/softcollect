// Shim: @common/utils/lyricUtils/kg —— 酷狗 KRC 歌词解密（移植自 LX, zlib + XOR）
...
const parseLyric = (str) => {
  ...
  let lxlyric = str.replace(/\[((\d+),\d+)\].*/g, (s) => {
    let result = s.match(/\[((\d+),\d+)\].*/);
    let time = parseInt(result[2]);
    let ms = time % 1000;
    time /= 1000;
    let m = parseInt(time / 60).toString().padStart(2, '0');
    time %= 60;
    let sec = parseInt(time).toString().padStart(2, '0');
    time = `${m}:${sec}.${ms}`;
    if (rlyric) rlyric[i] = `[${time}]${rlyric[i]?.join('') ?? ''}`;
    if (tlyric) tlyric[i] = `[${time}]${tlyric[i]?.join('') ?? ''}`;
    i++;
    return s.replace(result[1], time);
  });
